package urmat.zhenaliev.caravan.infrastructure
...
import urmat.zhenaliev.caravan.ITSpec
import urmat.zhenaliev.caravan.domain.{ItemRepository, _}
import urmat.zhenaliev.caravan.infrastructure.flyway.{FlywayProvider, MigrateResult}
...
object ItItemRepositorySpec extends ITSpec(Some("items")) {
  ...
  private def allItems: ZIO[ItemRepository, Throwable, List[Item]] =
    ItemRepository.getAll.mapError(_.asThrowable)
  ...
  private val suites = suite("Item Repository")(
    ...
    test("Should not allow to add wrong data to db") {
      ...
      } yield assert(error.toString)(
        startsWithString("RepositoryError(java.lang.NullPointerException"),
      )
    },
    // def getItem
