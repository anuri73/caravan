...
import urmat.zhenaliev.caravan.api.healthcheck.DbStatus
import urmat.zhenaliev.caravan.domain.{Item, ItemId}
...
trait JsonSupport {
  ...
  implicit val createItemRequestDecoder: JsonDecoder[CreateItemRequest] =
    DeriveJsonDecoder.gen[CreateItemRequest]
  implicit val createItemRequestEncoder: JsonEncoder[CreateItemRequest] =
    DeriveJsonEncoder.gen[CreateItemRequest]

  implicit val updateItemRequestDecoder: JsonDecoder[UpdateItemRequest] =
  ...
  implicit val emptyResponseEncoder: JsonEncoder[EmptyResponse] =
    DeriveJsonEncoder.gen[EmptyResponse]
}
